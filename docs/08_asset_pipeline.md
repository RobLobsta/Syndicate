...
| `module` | object | optional, and legal only on a `utility` part. `family` is one of the `ModuleFamily` set — `RADAR`, `AMMO_FEED`, `RADIATOR`, `REINFORCER`, `CLOAK`, `BOOSTER`, `SMOKE`, `REPAIR` — and fixes what the module does and whether it is **passive** or **active**; `charges` is activations before it is spent or `-1` for unlimited; `radiusM` is the extent of a family that has one and 0 for a module acting on its own vehicle. The numbers are stats: `moduleDurationS` and `moduleCooldownS` (D05-S4.5), so a damaged cloak stays lit for less time exactly as a damaged weapon fires slower. A passive module needs no block at all — a radiator is its `heatPerShot` stat and nothing else — and the block exists because an **active** one has a state no stat can represent. |
| `light` | object | optional, and present on a `light` part. `colorRgb` is what it emits, `intensity` its punctual strength, `coneOuterDeg` / `coneInnerDeg` the angle at which it has fallen to nothing and the angle within which it is at full, `rangeM` how far it reaches, `directionLocal` and `originLocal` where it points and where the light leaves the lens — both in the part's own space — and `castsBeam` whether it lights the world at all. **Cosmetic** (G6): nothing in the simulation reads it, and `game-core`'s loader deliberately does not parse it — the presentation layer does, exactly as it does the visual mesh and the morph target names. It is authored here rather than inferred by the renderer because where a lamp points is a property of the art (D15-R51). A `false` `castsBeam` is a lamp that glows and illuminates nothing, which is what a tail light is. |
...
├── parts/                                 # SHARED: modular parts, fitted to any vehicle
...
├── vehicles/
│   └── vehicle_medium_raider_01/
│       ├── assembly.json
│       └── parts/                         # VEHICLE-OWNED: cut from this car's art
│           ├── manifest.json              # what this vehicle is made of, and what each part is worth
│           ├── chassis_medium_raider_01/
│           │   ├── part.json
│           │   └── mesh.glb
│           └── panel_medium_raider_door_l_01/
│               ├── part.json
│               └── mesh.glb
├── structures/
│   └── str_gantry_sign_01/
...
**R14b.** A part is owned by exactly one of two places, and which one says what the part is *for*.

`assets/vehicles/<vehicleTypeId>/parts/<partTypeId>/` holds a **vehicle-owned** part: a chassis, a door, a wheel, a bonnet — geometry cut from one car's art, fitted to that car's slot graph, and meaningless on any other. Only that vehicle's own assembly may reference it, and an assembly that references another vehicle's part is A315. The rule is not bureaucracy: a door from a supercar bolted to a pickup is the wrong size, in the wrong place, with a mass derived from a body it is not on, and a single flat directory could neither prevent that nor detect it — nor stop two cars whose art both produced a `panel_door_l_01` from colliding.

`assets/parts/<partTypeId>/` holds a **shared** part: a weapon, a utility module, a universal accessory — authored once against a hardpoint and bolted onto whatever will take it. Any assembly may reference it. That is what makes a modular weapon modular, and it is why every prepared vehicle offers hardpoints its own art never supplied (D15-R49).

A vehicle-owned part is *not* a different schema. It is the same `part.json` in a different place, and the place is the ownership statement — a part never names a vehicle.

**R14c.** `assets/vehicles/<vehicleTypeId>/parts/manifest.json` describes the part set beside it: per part, what it is, what it weighs, what it costs, how it fails and how many triangles it took; plus the vehicle's totals and its hardpoints. **Nothing loads it and it is not a second source of truth** — every number in it is copied from a `part.json` written in the same run, and those are what the gate validates. It exists for the question neither a directory listing nor twenty separate files answers: what did the tool decide this vehicle is made of, and why.
...
| A216 | ERROR | A `weapon` block on a part that is not a `WEAPON`; or WARN when a `WEAPON` authors none |
