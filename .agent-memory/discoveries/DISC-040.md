...
## Details
Incremental insertion rests on one invariant: the faces a new point sees form a topological
disc, so replacing them with a fan leaves a closed surface. Two independent things break it
when the points are a 5 mm slab whose vertices sit on each other's face planes.
...
anyway. The surface is non-manifold from then on, and every later visibility test reads
through the hole.

This is not an epsilon that needs tuning. The distances genuinely *are* at the noise floor,
because the points genuinely *are* on each other's planes. Both fixes are structural: force the
...
like paranoia without the measurement. A future session reading `_repair_visible_cap` sees a
reclassification pass that never fires on any of the suite's clean geometry, and an insertion
that rolls back once in 24 shards, and will be tempted to simplify both away.
